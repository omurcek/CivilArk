...
try:
	screen_root.load_image("ongame_bg", menu_bg, width=550, height=192, alpha=150)
except Exception as e:
...
def settings():
	...
	while True:
		...
		screen_root.draw_text(text=exittext, x=logox, y=logoy+(screen_root.height/6)+310, width=textsize, height=textsize, color=textcolor, font="Font Squirrel")
		...
		for pos, _ in screen_root.events_checkmouse(events):
			...
			if logox <= x <= logox + (textsize * len(setlangtext)):
				if logoy + (screen_root.height / 6) + 270 <= y <= logoy + (screen_root.height / 6) + 270 + textsize:
					...
					screen_root.update()
					time.sleep(0.01)
					os.system("notepad.exe data/Detail/game_language.txt")
					exit()
			if logox <= x <= logox + (textsize * len(exittext)):
				if logoy + (screen_root.height / 6) + 310 <= y <= logoy + (screen_root.height / 6) + 310 + textsize:
					play_click()
					screen_root.draw_text(text=exittext, x=logox, y=logoy+(screen_root.height/6)+310, width=textsize, height=textsize, color=clickcolor, font="Font Squirrel")
					screen_root.update()
					time.sleep(0.01)
					return
